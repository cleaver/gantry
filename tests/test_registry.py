...
class TestRegisterProject:
    ...
    def test_register_project_rejects_duplicate_hostname(self, mock_registry, tmp_path):
        ...
        with pytest.raises(ValueError, match="already registered"):
            mock_registry.register_project(
                hostname="myproject", path=tmp_path / "other", port=5002
            )

    def test_register_project_validates_metadata_structure(
        ...
        project = mock_registry.register_project(
            hostname="testproj", path=project_path, port=5001
        )
    ...
    def test_register_project_creates_directory(
        self, mock_registry, tmp_gantry_home, tmp_path
    ):
    ...
    def test_register_project_atomic_write(
        self, mock_registry, tmp_gantry_home, tmp_path
    ):
...
class TestListProjects:
    ...
    def test_list_all_projects(self, mock_registry, tmp_path):
        ...
        for i in range(3):
            ...
            project_path.mkdir()
            mock_registry.register_project(
                hostname=f"project{i}", path=project_path, port=5001 + i
            )
...
class TestUnregisterProject:
    ...
    def test_unregister_cleans_up_directory(
        self, mock_registry, tmp_gantry_home, tmp_path
    ):
...
class TestUpdateProjectMetadata:
    ...
    def test_update_preserves_immutable_fields(self, mock_registry, tmp_path):
        ...
        mock_registry.update_project_metadata("myproject", services=["app"])
...
class TestGetRunningProjects:
    ...
    def test_get_running_projects_empty_when_none_running(
        self, mock_registry, tmp_path
    ):
