...
class TestIsPortAvailable:
    ...
    def test_port_unavailable_when_in_use(self, port_allocator, monkeypatch):
        ...
        monkeypatch.setattr(
            "gantry.port_allocator.socket.socket", lambda *args, **kwargs: MockSocket()
        )
...
class TestAllocatePort:
    ...
    def test_allocate_skips_allocated_ports(
        self, port_allocator, mock_registry, tmp_path
    ):
        ...
        project = mock_registry.register_project(
            hostname="project1", path=project_path, port=5001
        )
        mock_registry.update_project_metadata("project1", exposed_ports=[5001])
    ...
    @pytest.mark.slow
    def test_allocate_raises_error_when_no_ports_available(
        self, port_allocator, mock_registry, tmp_path
    ):
        ...
        for i in range(1000):  # 5000-5999 = 1000 ports
            ...
            project_path.mkdir()
            project = mock_registry.register_project(
                hostname=f"project{i}", path=project_path, port=5000 + i
            )
            mock_registry.update_project_metadata(
                f"project{i}", exposed_ports=[5000 + i]
            )
...
class TestGetProjectPort:
    ...
    def test_get_port_for_existing_project(
        self, port_allocator, mock_registry, tmp_path
    ):
        ...
        mock_registry.register_project(
            hostname="myproject", path=project_path, port=5001
        )
...
class TestDetectServicePorts:
    ...
    def test_detect_ports_mixed_syntax(self, port_allocator, tmp_path):
        ...
        compose_data = {
            "services": {
                ...
                "mailhog_web": {
                    "ports": [{"target": 8025, "published": 8025, "protocol": "tcp"}]
                },
            }
        }
        with open(compose_file, "w", encoding="utf-8") as f:
    ...
    def test_detect_ports_multiple_ports_per_service(self, port_allocator, tmp_path):
        ...
        compose_file = tmp_path / "docker-compose.yml"
        compose_data = {
            "services": {"app": {"ports": ["5001:5001", "5002:5002", "8080:80"]}}
        }
        with open(compose_file, "w", encoding="utf-8") as f:
            yaml.dump(compose_data, f)
    ...
    def test_detect_ports_services_without_ports(self, port_allocator, tmp_path):
        ...
        compose_file = tmp_path / "docker-compose.yml"
        compose_data = {
            "services": {"app": {"image": "nginx"}, "db": {"image": "postgres"}}
        }
        with open(compose_file, "w", encoding="utf-8") as f:
            yaml.dump(compose_data, f)
...
class TestCheckPortConflicts:
    ...
    def test_detect_conflicts_with_running_projects(
        self, port_allocator, mock_registry, tmp_path
    ):
    ...
    def test_skip_self_when_checking_conflicts(
        self, port_allocator, mock_registry, tmp_path
    ):
        ...
        mock_registry.update_project_metadata(
            "project1", status="running", exposed_ports=[5001, 5432]
        )
    ...
    def test_conflict_information_includes_service(
        self, port_allocator, mock_registry, tmp_path
    ):
    ...
    def test_multiple_conflicts(self, port_allocator, mock_registry, tmp_path):
        ...
        conflicts = port_allocator.check_port_conflicts(
            "newproject", [5001, 5432, 6379]
        )
...
class TestValidateStartupPorts:
    ...
    def test_validation_passes_when_no_conflicts(
        self, port_allocator, mock_registry, tmp_path
    ):
    ...
    def test_validation_raises_error_with_conflicts(
        self, port_allocator, mock_registry, tmp_path
    ):
...
class TestGetRunningProjectPorts:
    ...
    def test_get_running_project_ports(self, port_allocator, mock_registry, tmp_path):
        ...
        for i in range(3):
            ...
            mock_registry.update_project_metadata(
                f"project{i}", status=status, exposed_ports=[5001 + i, 5432 + i]
            )
    ...
    def test_get_running_project_ports_empty_when_none_running(
        self, port_allocator, mock_registry, tmp_path
    ):
        ...
        mock_registry.register_project("project1", project_path, port=5001)
        mock_registry.update_project_metadata(
            "project1", status="stopped", exposed_ports=[5001]
        )
...
class TestGetPortUsage:
    ...
    def test_get_port_usage_maps_ports_to_projects(
        self, port_allocator, mock_registry, tmp_path
    ):
        ...
        project1_path.mkdir()
        mock_registry.register_project("project1", project1_path, port=5001)
        mock_registry.update_project_metadata("project1", exposed_ports=[5001, 5432])
    ...
    def test_get_port_usage_handles_multiple_projects_same_port(
        self, port_allocator, mock_registry, tmp_path
    ):
