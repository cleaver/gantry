...
class TestInstallCommand:
    ...
    def test_get_install_command_no_os_release(self, mock_open, mock_system, dns_manager):
...
class TestDNSSetup:
    ...
    @patch("gantry.dns_manager.shutil.which")
    def test_setup_dns_fails_when_dnsmasq_not_installed(
        self, mock_which, dns_manager
    ):
    ...
    def test_setup_dns_fails_on_service_restart_error(
        ...
        mock_subprocess.side_effect = [
            ...
            subprocess.CalledProcessError(1, "sudo", stderr="Service failed"),  # systemctl
            subprocess.CalledProcessError(1, "sudo", stderr="Service failed"),  # service fallback
        ]
        
        with pytest.raises(DNSConfigError) as exc_info:
            dns_manager.setup_dns(require_sudo=True)
        
        assert "Could not restart dnsmasq" in str(exc_info.value) or "Failed to restart dnsmasq" in str(exc_info.value)
...
class TestDNSResolutionVerification:
    ...
    @patch("socket.gethostbyname")
    def test_test_dns_fails_with_resolution_error(self, mock_gethostbyname, dns_manager):
    ...
    @patch("socket.gethostbyname")
    def test_test_dns_fails_with_unexpected_error(self, mock_gethostbyname, dns_manager):
