...
class CaddyManager:
    ...
    def generate_caddyfile(self) -> str:
        ...
        """
        lines = ["# Auto-generated by Gantry", "{", "http_port 80", "https_port 443", "}"]

        projects = self._registry.list_projects()
        for project in projects:
            lines.append(f"\n# Project: {project.hostname}")
            lines.append(f"{project.hostname}.test {{")
            lines.append(f"  reverse_proxy localhost:{project.port}")
            lines.append("}")

            # Add routes for additional services if they exist
            for service_name, service_port in project.service_ports.items():
                # Avoid creating a route for the main service port if it's duplicated
                if service_port == project.port:
                    continue
                lines.append(f"{service_name}.{project.hostname}.test {{")
                lines.append(f"  reverse_proxy localhost:{service_port}")
                lines.append("}")
