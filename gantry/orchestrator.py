                ...
                for project in running_projects:
                    try:
                        ...
                        if status == "running":
                            ...
                            is_healthy = self._process_manager.health_check(project.hostname)
                            if not is_healthy:
                                logging.warning(f"Project '{project.hostname}' failed health check.")
                                # Future: Implement auto-restart policy here
                        
                    except Exception as e:
                        logging.error(f"Error monitoring project '{project.hostname}': {e}")
